      ...
      <div class="parameters-section">
        <div class="section-header">
          ...
          <div class="header-actions">
            ...
            <el-button type="primary" size="small" @click="saveParameters" :loading="savingParams">
...
import { Upload, Plus, Check, InfoFilled } from '@element-plus/icons-vue'
  ...
  try {
    ...
    if (conflictingNames.length > 0) {
      ...
      parameters.forEach(param => {
        const existingIndex = editParamsForm.value.parameters.findIndex(
          p => p.paramName === param.paramName
        )
        if (existingIndex !== -1) {
          // 覆盖现有参数的值
          editParamsForm.value.parameters[existingIndex].paramValue = param.paramValue
        } else {
          // 添加新参数
          editParamsForm.value.parameters.push(param)
        }
      })
    } else {
      // 如果没有冲突，直接追加所有参数
      // 如果当前只有一个空参数行，则替换；否则追加
      const isEmpty = editParamsForm.value.parameters.length === 1 && 
                      !editParamsForm.value.parameters[0].paramName && 
                      !editParamsForm.value.parameters[0].paramValue
      
      if (isEmpty) {
        editParamsForm.value.parameters.length = 0
        editParamsForm.value.parameters.push(...parameters)
      } else {
        editParamsForm.value.parameters.push(...parameters)
      }
    }
