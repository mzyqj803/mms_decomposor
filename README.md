...
- ✅ **软删除机制**: status字段标记删除状态，保留历史数据
- ✅ **删除过滤**: 已删除零部件自动从列表和关系中过滤
- ✅ **规格管理**: 零部件规格参数配置
- ✅ **关系管理**: 父子组件关系维护
- ✅ **工艺信息**: 零部件工艺流程配置
- ✅ **采购标识**: 自制/采购分类管理
...
- ✅ **非标组件恢复**: 自动恢复已删除的非标组件，更新名称和状态
- ✅ **事务隔离优化**: READ_COMMITTED隔离级别，消除快照隔离问题
- ✅ **乐观锁优化**: EntityManager.clear()和getReference()避免版本冲突
- ✅ **活动组件过滤**: 工艺分解只匹配status=1的活动组件
...
- ✅ **界面优化**: 隐藏未实现功能（克隆合同、生产计划、成本分析等），界面简洁清晰

### 2. 零部件管理 ✅
- ✅ **完整CRUD**: 创建、查看、编辑、删除零部件
- ✅ **软删除**: status字段标记删除状态(1=active, 0=deleted)，保留历史数据
- ✅ **删除过滤**: 已删除零部件自动从列表、关系和工艺分解中过滤
- ✅ **删除优化**: 删除时不检查引用和子组件，直接软删除
...
- ✅ **乐观锁优化**: EntityManager.clear()清除缓存，getReference()获取代理避免版本冲突
- ✅ **非标组件自动创建**: 识别包含`~`的componentCode，自动创建非标组件
- ✅ **非标组件恢复**: 自动恢复已删除(status=0)的非标组件，更新名称和状态
- ✅ **非标组件标记**: 自动添加nonStandardPartFlag=1到components_spec
- ✅ **活动组件过滤**: 只匹配status=1的活动组件，已删除组件自动排除
- ✅ **三层并发防护**: 
...
**核心业务流程**
- ✅ **合同管理**: 完整的CRUD、逻辑删除、参数管理、装箱单上传、界面优化
- ✅ **零部件管理**: CRUD、软删除(status字段)、删除过滤、规格管理、关系管理、工艺配置、缓存优化
...
- ✅ **工艺分解**: 
  ...
  - 乐观锁优化（EntityManager.clear() + getReference()）
  - 三层并发防护（synchronized + 编程式事务 + 唯一约束）
  - 非标组件自动创建（线程安全）
  - 非标组件自动恢复（status=0 → status=1）
  - 活动组件过滤（只匹配status=1）
  - ERP代码自动匹配
  - 合并分解表PDF生成
  - 选择性导出（支持特定箱包）
